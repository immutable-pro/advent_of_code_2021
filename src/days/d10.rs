...
pub fn part1() {
    ...
    let result = read_file_lines("input/10.txt")
        .iter()
        .fold(0, |acc, chunk| {
            let mut stack = Vec::<char>::new();
            let mut points = 0;
            chunk.chars().find(|c| {
                if let Some(closing) = pairs.get(c) {
                    stack.push(*closing);
                    false
                } else {
                    match stack.pop() {
...
pub fn part2() {
    ...
    let mut scores = read_file_lines("input/10.txt")
        .into_iter()
        .map(|chunk| {
            let mut stack = Vec::<char>::new();
            let mut error = false;
            ...
            if !error {
                stack.iter().rev().fold(0, |sum, c| sum * 5 + score[c])
            } else {
                0
            }
        })
